...
(function () {
    ...
    function onCourtroomJoin() {
        ...
        } else if (last_poseId && last_characterId && _CE_.$vue.$store.getters["auth/isLoggedIn"]) {
        ...
        _CE_.$vue.$watch("$store.state.courtroom.dialogs.presentDialog", (newValue, oldValue) => {
            setTimeout(() => {
                ...
                if (ui.presentDialog.checkItem.evidenceType && ui.MediaViewer.type !== ui.presentDialog.checkItem.evidenceType) {
        ...
        _CE_.Uploader = {
            ...
            upload(file, callbackSuccess, callbackError, callbackProgress) {
        ...
        _CE_.chatTTS = {
            ...
            mutationWatcherAction(mutation, state) {
                if (mutation.type == "courtroom/APPEND_CHAT_FRAME") {
                    ...
                    if (!chatFrame.frame.text) return;
            ...
            idToUtterance(id) {
                ...
                utterance.rate = 0.8 + ((totalValue * 4 % 9) * 0.1); // rate from 0.8 to 1.6
                utterance.pitch = (totalValue * 6 % 17) * 0.1; // pitch from 0 to 1.7
